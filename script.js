...
function showCategories() {
    ...
    Object.keys(data).forEach(cat => {
        html += `
            <div class="category-card" onclick="showSpecialityViewer('${escapeHtml(cat)}')">
                <div class="category-icon">
                    <img src="assets/${escapeHtml(cat)}.jpeg" alt="${escapeHtml(cat)}" 
                         onerror="this.style.display='none'; this.parentElement.innerHTML='${cat.substring(0, 2).toUpperCase()}';">
                </div>
...
// ======================================
// SPECIALITY VIEWER (Shows speciality image + product names)
// ======================================
function showSpecialityViewer(category) {
    currentView = 'speciality-viewer';
    ...
    renderSpecialityViewer();
}

function renderSpecialityViewer() {
    const specialityImage = `assets/${currentCategory}.jpeg`;
    ...
    const html = `
        ...
        <div class="viewer-wrapper">
            <div class="viewer-card">
                <img src="${escapeHtml(specialityImage)}" alt="${escapeHtml(currentCategory)}"
                     onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22400%22 height=%22300%22%3E%3Crect fill=%22%231e1e1e%22 width=%22400%22 height=%22300%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 fill=%22%237a8f6e%22 font-size=%2224%22%3E${escapeHtml(currentCategory)}%3C/text%3E%3C/svg%3E'">
            </div>
            
            <div class="viewer-sidebar">
                <ul class="viewer-product-list">
                    ${filteredProducts.map((p, i) => `
                        <li onclick="showProductViewer(${i})">
                            ${escapeHtml(p.name)}
                        </li>
                    `).join('')}
                </ul>
...
function closeSpecialityViewer() {
    const catalogHeader = document.getElementById('catalogHeader');
    const catalogContainer = document.getElementById('catalogContainer');
    const body = document.body;
...
// ======================================
// RENDER PRODUCTS (for All Products view)
// ======================================
        ...
        html += `
            <div class="product-card" onclick="showProductViewer(${idx})">
                <img src="${escapeHtml(product.img)}"
                     ...
                     onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22250%22 height=%22200%22%3E%3Crect fill=%22%231e1e1e%22 width=%22250%22 height=%22200%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 fill=%22%237a8f6e%22 font-size=%2216%22%3ENo Image%3C/text%3E%3C/svg%3E'">
...
// ======================================
// PRODUCT VIEWER (Shows individual product)
// ======================================
        ...
        <div class="viewer-wrapper">
            <div class="viewer-card">
                <img src="${escapeHtml(product.img)}" alt="${escapeHtml(product.name)}"
                     onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22400%22 height=%22300%22%3E%3Crect fill=%22%231e1e1e%22 width=%22400%22 height=%22300%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 fill=%22%237a8f6e%22 font-size=%2220%22%3ENo Image%3C/text%3E%3C/svg%3E'">
            </div>
...
function closeViewer() {
    ...
    if (currentView === 'viewer' && currentCategory !== 'All Products') {
        // Return to speciality viewer if we came from there
        showSpecialityViewer(currentCategory);
...
function goBack() {
    ...
    } else if (currentView === 'speciality-viewer') {
        closeSpecialityViewer();
...
// GLOBAL EXPORTS
// ======================================
window.showSpecialityViewer = showSpecialityViewer;
